...
def _create_zip_folder_list(dags_directory: str) -> Tuple[str, List[str]]:
    temp_dir = tempfile.mkdtemp()
...
def upload_zip_folders_to_composer(dags_directory: str, bucket_name: str) -> None:
    temp_dir, folders = _create_zip_folder_list(dags_directory)
    ...
    if len(folders) > 0:
        ...
        for folder in folders:
            ...
            if "dags" in current_directory:
                folder = folder.replace(f"{temp_dir}/", "dags/")
            else:
                folder = folder.replace(f"{temp_dir}/", "../dags/")
            ...
            try:
                blob.upload_from_filename(folder, content_type='application/zip')
            except:
                blob.upload_from_filename(folder)
            print(f"Folder {folder} uploaded to {bucket_name}/{folder}.")
...
if __name__ == "__main__":
    ...
    # upload_local_directory_to_gcs(args.dags_directory, args.dags_bucket, args.dags_directory)
    upload_zip_folders_to_composer(args.dags_directory, args.dags_bucket)
